...
const ComercioForm = ({ onCreationSuccess, onCancel }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Solo crear comercio (el menú se crea automáticamente desde el backend)
      const comercioResponse = await api.post('/comercios/registrar', formData, {
        headers: { Authorization: `Bearer ${token}` },
      });

      Swal.fire('¡Éxito!', 'Comercio creado correctamente con menú automático.', 'success');
      onCreationSuccess();

    } catch (error) {
      console.error('Error al crear comercio:', error.response?.data || error);
      Swal.fire(
        'Error',
        error.response?.data?.msg || 'Ocurrió un error al crear el comercio.',
        'error'
      );
    } finally {
      setLoading(false);
